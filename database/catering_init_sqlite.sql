...
DROP TABLE IF EXISTS `EventInfoNotes`;

DROP TABLE IF EXISTS `EventInfoLocations`;

DROP TABLE IF EXISTS `EventInfoInfo`;

DROP TABLE IF EXISTS `EventInfoTypeOfServices`;

DROP TABLE IF EXISTS `StaffEventInfos`;

DROP TABLE IF EXISTS `Contacts`;

DROP TABLE IF EXISTS `ContactedStaff`;

DROP TABLE IF EXISTS `StaffMembers`;

DROP TABLE IF EXISTS `HolidaysOfStaffMember`;

DROP TABLE IF EXISTS `HolidayTableRequest`;

DROP TABLE IF EXISTS `AvailabilityTable`;

DROP TABLE IF EXISTS `EnumRoles`;
...
CREATE TABLE
    `EventInfoInfo` (
        `event_id` INTEGER,
        `info_name` TEXT,
        PRIMARY KEY (`event_id`, `info_name`),
        FOREIGN KEY (`event_id`) REFERENCES `Events` (`id`)
    );
...
CREATE TABLE
    `EventInfoLocations` (
        `event_id` INTEGER,
        `location_name` TEXT,
        PRIMARY KEY (`event_id`, `location_name`),
        FOREIGN KEY (`event_id`) REFERENCES `Events` (`id`)
    );

CREATE TABLE
    `EventInfoTypeOfServices` ( 
        `event_id` INTEGER,
        `service_type` TEXT,
...
CREATE TABLE
    `StaffEventInfos` (
        `event_id` INTEGER,
        `numbers_of_wait_staff` INTEGER NOT NULL DEFAULT 0,
        `numbers_of_kitchen_staff` INTEGER NOT NULL DEFAULT 0,
        PRIMARY KEY (`event_id`),
        FOREIGN KEY (`event_id`) REFERENCES `Events` (`id`)
    );
...
CREATE TABLE 
    `Contacts` (
        ...
        CHECK( COALESCE(email, '') <> '' OR COALESCE(whatsapp, '') <> '' )
    );
...
CREATE TABLE 
    `StaffMembers` (
        `contact_id` INTEGER,
        `permanent_staff` BOOLEAN NOT NULL,
        `kitchen_staff` BOOLEAN NOT NULL,
        `numbers_of_holidays_days` INTEGER DEFAULT 20,
        PRIMARY KEY (`contact_id`),
        FOREIGN KEY (`contact_id`) REFERENCES `Contacts` (`user_id`)
    );

CREATE TABLE 
    `HolidaysOfStaffMember` (
        `from_date` TEXT NOT NULL,
        `to_date` TEXT NOT NULL,
        `staff_member_id` INTEGER,
        PRIMARY KEY (`from_date`, `to_date`, `staff_member_id`),
        FOREIGN KEY (`staff_member_id`) REFERENCES `StaffMembers` (`contact_id`)
    );
...
CREATE TABLE 
    `AvailabilityTable` (
        `date` TEXT NOT NULL,
        `staff_member_id` INTEGER,
        PRIMARY KEY (`date`, `staff_member_id`),
        FOREIGN KEY (`staff_member_id`) REFERENCES `StaffMembers` (`contact_id`)
    );
...
INSERT OR IGNORE INTO EnumRoles (id, role) VALUES
(0,'CUOCO'),
(1, 'CHEF'),
(2, 'ORGANIZZATORE'),
(3, 'SERVIZIO'),
