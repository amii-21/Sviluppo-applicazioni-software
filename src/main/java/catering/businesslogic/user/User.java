...
public class User {
    ...
    public User(String username,Contact contact) {
        id = 0;
    ...
    public boolean isCook() {
        return roles.contains(enumRole.CUOCO);
    }

    public boolean isChef() {
        return roles.contains(enumRole.CHEF);
    }
    public boolean isOwner(){ 
         return roles.contains(enumRole.PROPRIETARIO);
    }
    public boolean isOrganizer(){
        return roles.contains(enumRole.ORGANIZZATORE); 
    }
     ...
     */
    public boolean addRole(enumRole role) {
     ...
     */
    public boolean removeRole(enumRole role) {
    ...
    private static void loadRolesForUser(User u) {
        ...
        PersistenceManager.executeQuery(roleQuery, new ResultHandler() {
            ...
            public void handle(ResultSet rs) throws SQLException {
                ...
                switch (role) {
                    case 0:
                        u.roles.add(User.enumRole.CUOCO);
                        break;
                    case 1:
                        u.roles.add(User.enumRole.CHEF);
                        break;
                    case 2:
                        u.roles.add(User.enumRole.ORGANIZZATORE);
                        break;
                    case 3:
                        u.roles.add(User.enumRole.SERVIZIO);
                        break;
                    case 4:
                        u.roles.add(User.enumRole.PROPRIETARIO);
                        break;
                }
            }
    ...
    private void saveUserRoles() {
        ...
        for (enumRole role : roles) {
            int roleId = getRoleStringId(role);
    ...
    /**
     * Converts enumRole enum to string ID for database
     */
    private int getRoleStringId(enumRole role) {
        switch (role) {
            case CUOCO:
                return 0;
            case CHEF:
